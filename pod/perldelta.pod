...
PERL_HASH_SEED no longer accepts an integer as a parameter, instead the value
is expected to be a binary value encoded as a hex string, such as
"0xf5867c55039dc724".  This is to make the infrastructure support hash seeds of
arbitrary lengths which might exceed that of an integer.  (SipHash uses a 16
byte seed).
...
Repetitive runs of the same program should produce the same output every time.
...
Optimization of hashes in boolean context extended to include C<scalar(%hash)>,
...
# XXX is this todo or should it be removed? -- xdg, 2013-05-08
...
# XXX not a platform note -- xdg, 2013-05-08
...
# XXX is this deprecated? -- xdg, 2013-05-08
